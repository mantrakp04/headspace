...
VENDOR = ROOT / 'desktop/vendor/sparkle'
CONFIG = ROOT / 'desktop/release.json'
...
def bundle_info(cfg, development=False):
    return {
        'CFBundleExecutable': 'Headspace', 'CFBundleIdentifier': cfg['bundle_id'],
        'CFBundleName': 'Headspace', 'CFBundleDisplayName': 'Headspace',
...
def build(cfg, development=False, destination=None):
    ensure_sparkle(cfg)
    bundle = destination or CACHE / 'Build/Headspace.app'
    ...
    with tempfile.TemporaryDirectory(prefix='headspace-build-', dir=bundle.parent) as tmp:
        ...
        run('npm', 'exec', 'vite', 'build', '--', '--config', 'desktop/vite.config.ts')
        sdk = run('xcrun', '--sdk', 'macosx', '--show-sdk-path', capture=True)
        run('xcrun', '--sdk', 'macosx', 'swiftc', '-sdk', sdk, '-O', '-target', f"arm64-apple-macos{cfg['minimum_macos']}",
            ROOT / 'desktop/native/Headspace.swift', '-F', VENDOR,
        ...
        shutil.copytree(ROOT / 'desktop/build/web', contents / 'Resources/web')
        shutil.copy2(ROOT / 'desktop/assets/Headspace.icns', contents / 'Resources')
        framework = contents / 'Frameworks/Sparkle.framework'
        shutil.copytree(VENDOR / 'Sparkle.framework', framework, symlinks=True)
        (contents / 'Info.plist').write_bytes(plistlib.dumps(bundle_info(cfg, development)))
...
def prepare(cfg, profile):
    ...
    if public_key != cfg['sparkle_public_key']:
        raise ValueError('The update signing key in Keychain does not match desktop/release.json.')
    assert_new_release(cfg)
    run('xcrun', 'notarytool', 'history', '--keychain-profile', profile, '--output-format', 'json', capture=True)
    run('npm', 'test')
    run('npm', 'run', 'typecheck')
    run('npm', 'run', 'lint')
    run('python3', '-m', 'unittest', 'discover', '-s', 'desktop/scripts', '-p', 'test_*.py')
    ...
    with tempfile.TemporaryDirectory(prefix='headspace-appcast-') as tmp:
        ...
        notes = ROOT / 'desktop/release-notes' / f"{cfg['version']}.md"
...
def publish(cfg):
    ...
    run('gh', 'release', 'create', tag, '--repo', cfg['repository'], '--target', sha, '--draft',
        ...
        ROOT / 'desktop/release-notes' / f"{cfg['version']}.md",
...
def main():
    ...
    cfg = config()
    if args.development and args.command != 'build':
        parser.error('--development is only valid for local builds.')
    if args.command == 'build':
        build(cfg, args.development)
    else:
