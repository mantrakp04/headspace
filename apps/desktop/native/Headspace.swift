...
#if HEADSPACE_HMR
let rendererPort = 4383
let rendererOrigin = "http://127.0.0.1:4383"
#else
let rendererPort = 4382
let rendererOrigin = localOrigin
#endif
...
final class LocalServer {
    ...
    private func send(_ connection: NWConnection, _ status: Int, _ mime: String, _ body: Data) {
        let headers = "HTTP/1.1 \(status) \(status == 200 ? "OK" : "Error")\r\nContent-Type: \(mime)\r\nContent-Length: \(body.count)\r\nCache-Control: no-store\r\nX-Content-Type-Options: nosniff\r\nReferrer-Policy: no-referrer\r\nContent-Security-Policy: default-src 'self'; script-src 'self' https://sdk.scdn.co; frame-src https://sdk.scdn.co; media-src 'self' blob: https:; style-src 'self' 'unsafe-inline'; img-src 'self' https://i.scdn.co https://mosaic.scdn.co https://image-cdn-ak.spotifycdn.com https://image-cdn-fa.spotifycdn.com https://image-cdn-ak.spotifycdn.com data:; connect-src 'self' https://api.spotify.com https://accounts.spotify.com https://api.hexclave.com https://r.hexclave.com; frame-ancestors 'none'; base-uri 'none'\r\nConnection: close\r\n\r\n"
...
final class AppDelegate: NSObject, NSApplicationDelegate, WKScriptMessageHandler, WKNavigationDelegate, NSMenuItemValidation {
    ...
    private var lastVisualDiagnosticTime: TimeInterval = 0
    private lazy var audioCapture = SystemAudioCapture(
        onFrame: { [weak self] frame in self?.emitAudio(frame) },
        onStatus: { [weak self] state, message in self?.emit("headspace-audio-status", ["state": state.rawValue, "message": message]) }
    )
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.title = Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String ?? "Headspace"
        ...
        do { try server.start { [weak self] error in
            ...
            self.webView.load(URLRequest(url: URL(string: rendererOrigin)!))
            self.window.makeKeyAndOrderFront(nil); NSApp.activate(ignoringOtherApps: true)
        } } catch { NSApp.terminate(nil) }
    }
    func applicationShouldTerminateAfterLastWindowClosed(_ sender: NSApplication) -> Bool { true }
    func applicationWillTerminate(_ notification: Notification) { audioCapture.shutDown() }
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy)->Void) {
        ...
        if url.scheme == "http",url.host == "127.0.0.1",url.port == rendererPort { decisionHandler(.allow);return }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard message.frameInfo.isMainFrame, message.frameInfo.securityOrigin.host == "127.0.0.1", message.frameInfo.securityOrigin.protocol == "http", message.frameInfo.securityOrigin.port == rendererPort, let body=message.body as? [String:Any],let id=body["id"] as? Int,let method=body["method"] as? String else { return }
        let args=body["args"] as? [String:Any] ?? [:]
        switch method {
        case "startAudioCapture": audioCapture.start();reply(id,true)
        case "stopAudioCapture": audioCapture.stop();reply(id,true)
    ...
    private func emit(_ name:String,_ value:Any) { guard let data=try? JSONSerialization.data(withJSONObject:["name":name,"value":value]),let json=String(data:data,encoding:.utf8) else{return};webView.evaluateJavaScript("(()=>{const e=\(json);window.dispatchEvent(new CustomEvent(e.name,{detail:e.value}));})()",completionHandler:nil) }
    private func emitAudio(_ frame: AudioAnalysisFrame) {
        emit("headspace-audio", frame.payload)
        let now = ProcessInfo.processInfo.systemUptime
        guard CommandLine.arguments.contains("--audio-diagnostics"), now - lastVisualDiagnosticTime > 1 else { return }
        lastVisualDiagnosticTime = now
        webView.evaluateJavaScript("JSON.stringify({renderer:document.querySelector('.screen canvas')?.dataset.renderer,scene:document.querySelector('.screen canvas')?.getAttribute('aria-label'),cones:[...document.querySelectorAll('[data-cone]')].map(e=>e.getAttribute('transform'))})") { value, error in
            guard error == nil, let result = value as? String, let data = ("headspace-visual " + result + "\n").data(using: .utf8) else { return }
